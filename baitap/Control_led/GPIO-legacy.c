...
static long m_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch(cmd){
        case LED_ON:
            ...
            reg &= ~(7 << ((GPIO_PIN  % 10) *3)); //clear current function
            reg |= (1 << ((GPIO_PIN  % 10)* 3)); //set as output
            iowrite32(reg, gpio_base + GPIO_FSEL_OFFSET + (GPIO_PIN  / 10)*4); 
            return 0;

            //03. Set GPIO27 to HIGH
            iowrite32(1 << GPIO_PIN, gpio_base + GPIO_SET_0_OFFSET);
        ...
        case LED_OFF:
            ...
            //03. Set GPIO27 to HIGH
            iowrite32(1 << GPIO_PIN, gpio_base + GPIO_CLR_0_OFFSET);
...
{   
    ...
    if(alloc_chrdev_region(&mdev.dev_num, 0, 1, "gpio-legacy") < 0) {
    ...
    if ((mdev.m_class = class_create("gpio_class")) == NULL){
        pr_err("Cannot create the struct  class for my device\n");
        goto rm_device_numb;
    }

    /* 04. Creating device*/
    if((device_create(mdev.m_class, NULL, mdev.dev_num, NULL, "gpio_device_legacy")) == NULL){
